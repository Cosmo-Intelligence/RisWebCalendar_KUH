...
//時間と検査室に関連付けられた検査一覧
HashMap<String, HashMap<ExamRoom, List<OrderCalendarItem>>> timeRoomOrderCalenderMap = new HashMap<String, HashMap<ExamRoom, List<OrderCalendarItem>>>();
//// 時間と日付に関連付けられた検査一覧
...
if( request.getAttribute(Parameters.KENSALIST) != null ){
	timeRoomOrderCalenderMap = (HashMap<String, HashMap<ExamRoom, List<OrderCalendarItem>>>) request
...
<%-- 2015.02.27 Del S.Matsumoto@CST Start KANA-C1-001 オーダ表示方法の変更 --%>
<%-- <%!// 2013.11.10 Add Yk.Suzuki@CIJ Start NCC_C-2-TKC-R005
/**
 * 対象日付の合計値を返す。
 * @param timeDateOrderCalendarMap カレンダーMAP情報
 * @param date 対象日付
 * @return 合計値
 */
int getDaySum(HashMap timeDateOrderCalendarMap, Integer date)
{
	int sum = 0;
	// 時間枠ごとのループ
	for (Object timeFrame : timeDateOrderCalendarMap.keySet()) {
		// 該当時間の時間情報を取得
		HashMap dateOrderCalendarMap = (HashMap)timeDateOrderCalendarMap.get(timeFrame);
		if (null== dateOrderCalendarMap) continue;

		List orderCalendarItemList = (ArrayList)dateOrderCalendarMap.get(date);
		if (null == orderCalendarItemList) continue;

		for(int index=0; index < orderCalendarItemList.size(); index++){
			OrderCalendarItem item = (OrderCalendarItem)orderCalendarItemList.get(index);
...
// 2013.11.10 Add Yk.Suzuki@CIJ End   NCC_C-2-TKC-R005
 %> --%>
<%-- 2015.02.27 Del S.Matsumoto@CST End KANA-C1-001 オーダ表示方法の変更 --%>
...
<div id="dateblock">
	...
	for(Iterator<ExamRoom> ite = roomList.iterator(); ite.hasNext();) {
		...
		int roomSum = calenderAction.getRoomSum(timeRoomOrderCalenderMap, roomBean);

		if ((roomBean.getExamRoomName() == null) || (roomBean.getExamRoomName().length() == 0)){
			if (roomSum == 0){
				//非表示、オーダ0件の時
			} else {
				roomBean.setExamRoomID("0000");
				dspRoom = "未割当" + "&nbsp;" + "(" + String.valueOf(roomSum) + ")";
				dspRoom += orderCountUnit;
			}
		}else{
			// 件数を検査室の後に付加する
			dspRoom = roomBean.getExamRoomName() + "&nbsp;" + "(" + String.valueOf(roomSum) + ")";
			dspRoom += orderCountUnit;
		}
		//if ((roomBean.getExamRoomName() == null) || (roomBean.getExamRoomName().length() == 0)){
		//	//非表示、オーダ0件の時
...
%>
				<td id="header<%=roomBean.getExamRoomID()%>">
					<div class="date">
						<%=dspRoom%>
						<span class="dateColNum" style="display: none;"><%=colNo%></span>
					</div>
				</td>
				<%--
<%
	...
	for(int i = 0; i < dateList.size(); i++){
		...
		// 2013.11.10 Add Yk.Suzuki@CIJ Start NCC_C-2-TKC-R005
		// 日毎の件数を求める
		int daySum = getDaySum(timeDateOrderCalenderMap, (Integer)dateList.get(i));

		// 件数を日付の後に付加する
		String daySumStr = String.valueOf(daySum);
		yMDE += "&nbsp;"+ daySumStr;
		// 2013.11.10 Add Yk.Suzuki@CIJ End   NCC_C-2-TKC-R005
%>
       <td id="header<%= strDate %>" class="<%= domClass %>">
        <div class="date">
<!-- 2013.11.19 Mod Start NCC_C-2-TKC-R005 -->
         <%= yMDE %><%= orderCountUnit %>
         <!-- <%= yMDE %> -->
<!-- 2013.11.19 Mod End   NCC_C-2-TKC-R005 -->
         <span class="dateColNum" style="display: none;"><%=i%></span>
         <span class ="datetime" style="display: none;"><%= defStrDate %></span>
       	</div>
       </td>
--%>
				<%-- 2015.02.27 Mod S.Matsumoto@CST End KANA-C1-001 オーダ表示方法の変更 --%>
...
<div id="timeblock" onscroll="SyncCalendarScroll()">
	...
	for(int i = 0; i < timeFrameList.size()-1; i++){
		...
		// 現在見ている時間枠内の検査を取得
		// 2015.02.27 Mod S.Matsumoto@CST Start KANA-C1-001 オーダ表示方法の変更
		HashMap<ExamRoom, List<OrderCalendarItem>> roomOrderCalenderMap = timeRoomOrderCalenderMap.get(timeFrame);
...
%>
		...
		colNo = 0;
 		for(int j = 0; j < roomList.size(); j++) {
			ExamRoom roomBean = roomList.get(j);
			// 現在見ている時間、検査室に一致する検査一覧を取得。
			List<OrderCalendarItem> orderCalendarItemList =
			(ArrayList<OrderCalendarItem>)roomOrderCalenderMap.get(roomBean);
...
%>
				<%--  2015.02.27 Mod S.Matsumoto@CST Start KANA-C1-001 オーダ表示方法の変更 --%>
				<td id="data<%=roomBean.getExamRoomID()%>">
					<%--<td id="data<%=date%>" class="<%=domClass%>"> --%> <%--  2015.02.27 Mod S.Matsumoto@CST End KANA-C1-001 オーダ表示方法の変更 --%>
					<%
			} else {
%> <%--  2015.02.27 Mod S.Matsumoto@CST Start KANA-C1-001 オーダ表示方法の変更 --%>
				<td>
					<%-- <td class="<%=domClass%>"> --%> <%--  2015.02.27 Mod S.Matsumoto@CST End KANA-C1-001 オーダ表示方法の変更 --%>
					<%
			}
%> <%--  2015.02.27 Mod S.Matsumoto@CST Start KANA-C1-001 オーダ表示方法の変更 --%>
					<span class="date" style="display: none;"><%=roomBean.getExamRoomID()%></span>
					<div class="data">
						<span class="datetime" style="display: none;"><%=roomBean.getExamRoomID()%><%=strDisplayTimeFrame%></span>
