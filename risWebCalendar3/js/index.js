...
var COOKIE_PATIENT_BYOUTOU = "patientByoutou";
...
// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
//テキストボックスに表示する患者病棟
var patientByoutouData = new Array();
// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
// テキストボックスに表示する患者入外
...
$(function() {
	...
	// 患者病棟ダイアログの初期化
	if (!$.support.style)
		dialogWidth = $('#patientByoutouDialog').width() * 1.5;
	$('#patientByoutouDialog').dialog({
		bgiframe: true,
		autoOpen: false,
		width: dialogWidth,
		modal: true,
		...
		buttons: {
			'OK': function() {
				var elements = $('#patientByoutouDialog .byoutou');
				delete patientByoutouData;
				patientByoutouData = new Array();
				elements.each(function(i, elem){
					if(elem.checked){
						patientByoutouData.push({"name" : elem.name, "id" : elem.value});
					}
				});
				var str = "";
				var ids = ""
				for(i=0;i<patientByoutouData.length;i++){
					str += patientByoutouData[i].name;
					ids += patientByoutouData[i].id;
					if(i != patientByoutouData.length - 1) {
						str+= ", ";
						ids+= ":";
					}
				}
				$('#patientByoutouText').val(str);
				$('#patientByoutouID').val(ids);
				$(this).dialog('close');
				searchOrder();
			},
			// 「Cancel」ボタンがクリックされた時には、
	...
	// idが「openPatientByoutouDialog」のリンクがクリックされた時に、
	// ダイアログボックスを開くようにする
	$('#openPatientByoutouDialog').bind('click', function() {
		var x = $('#patientByoutouText').position().left + $('#SearchBlockCenter').position().left;
		var y = $('#patientByoutouText').height() + $('#patientByoutouText').position().top + 5 + $('#SearchBlockCenter').position().top;
		$('#patientByoutouDialog').dialog('option','position',[x,y]);
		var elements = $('#patientByoutouCheckBoxes input:checkbox');
		elements.each(function(j, elem){
			elem.checked = false;
		});
		for(i=0;i<patientByoutouData.length;i++){
			elements.each(function(j, elem){
				if(elem.value == patientByoutouData[i].id){
					elem.checked = true;
				}
			});
		}
		// ダイアログボックスを開く
		$('#patientByoutouDialog').dialog('open');
	});
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
...
function initCalendarItem() {
	...
	for (var i = 0; i < divNodes.length; i++) {
		...
		// 2019.09.02 Del H.Taira@COSMO Start KUMA205-10-002
		// 枠リスト右クリックメニュー表示
//		$(div).find("div.detailData").each(function() {
//
...
//			});
//		});
		// 2019.09.02 Del H.Taira@COSMO End   KUMA205-10-002
		// 2016.12.20 Add T.onoda@Cosmo End KUMA205-3-CAL005 予約時刻、予定検査室の変更
...
//2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
function getPatientByoutou() {
	return searchPatientByoutou;
}
//2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
...
function changeCalendar(date) {
	...
	// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
	var patientByoutou = getPatientByoutou();
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
	...
	if (pair == null || pair == "" || pair == "action=kensasitu_v") {
	...
	if (byoutou != null && byoutou.length > 0)
		url += "&byoutou=" + byoutou;
	// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
	if (patientByoutou != null && patientByoutou.length > 0)
		url += "&patientByoutou=" + patientByoutou;
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
...
function loadOrderDetail(url, risIDParam) {
	...
	var pair=location.search.substring(1);
	// 2019.09.02 Mod H.Taira@COSMO Start KUMA205-10-003
	if (pair == null || pair == "" || pair == "action=kensasitu_v" || pair == "action=kensasitu_h") {
...
function searchOrder() {
	...
	// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
	searchPatientByoutou = $("#patientByoutouID").val();
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
	...
	// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
	$.cookie(COOKIE_PATIENT_BYOUTOU, searchPatientByoutou, { expires: COOKIE_EXPIRES });
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
...
//2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
function insertPatientByoutou() {
	var form = document.getElementById('patientByoutouCheckBoxes');
	var html = "";
	var j = 0;
	html+="<table>"
	for(i=0;i<byoutouArray.length;i++){
		if(j == 0) html += "<tr>";
		html += "<td>";
		html += "<input type=\"checkbox\" onclick='changeAllButton(this, \"selectAllByoutou\")' id=\"checkboxPatientByoutou"+i+"\" class=\"byoutou\" name=\"" + byoutouArray[i].name + "\" value = \"" + byoutouArray[i].id + "\" ><label for=\"checkboxPatientByoutou"+ i + "\">"+ byoutouArray[i].name + "</label></input>";
		html += "</td>";
		if(j++ == 2) {
			html += "</tr>";
			j = 0;
	...
	var selectAll = "$('#patientByoutouCheckBoxes table input:checkbox').attr('checked', this.checked);";
	html += "</table><input type=\"checkbox\" id=\"selectAllByoutou\" onclick=\"" + selectAll + "\"><label for=\"selectAllByoutou\">全選択/全解除</label></input>";
	form.innerHTML = html;
}
//2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
...
function setValuesFromCookie() {
	...
	// 2019.09.02 Add H.Taira@COSMO Start KUMA205-10-001
	var patientByoutouIDs = new Array();
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
	...
	str = "";
	newIdList = "";
	if ($.cookie(COOKIE_PATIENT_BYOUTOU)) {
		$('#patientByoutouID').val($.cookie(COOKIE_PATIENT_BYOUTOU));
		patientByoutouIDs = $('#patientByoutouID').val().split(":");
		for (i = 0; i < byoutouArray.length; i++) {
			for (j = 0; j < patientByoutouIDs.length; j++) {
				if (byoutouArray[i].id == patientByoutouIDs[j]) {
					patientByoutouData.push({"name" : byoutouArray[i].name, "id" : byoutouArray[i].id});
					str += byoutouArray[i].name + ", ";
					newIdList +=  byoutouArray[i].id +":";
				}
			}
		}
		$('#patientByoutouText').val(str.substr(0, str.length - 2));
		$('#patientByoutouID').val(newIdList.substr(0,newIdList.length-1));
	}
	// 2019.09.02 Add H.Taira@COSMO End   KUMA205-10-001
